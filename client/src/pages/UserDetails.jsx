...
import { User, Calendar, Phone, MapPin, ClipboardList, Upload, Heart, Mic, Sparkles, Shield, Activity } from 'lucide-react';
...
const VoiceButton = ({ listening, onClick, fieldName }) => (
  <button
    ...
    aria-label={listening ? 'Stop recording' : `Start voice input for ${fieldName}`}
  >
    <Mic size={18} className={`transition-transform duration-300 ${listening ? 'animate-bounce' : ''}`} />
  </button>
);
...
export function UserDetails() {
  ...
  const handleFileUpload = (e) => {
    setFormData(prev => ({ ...prev, medicalCertificates: e.target.files[0] }));
  };
  ...
  React.useEffect(() => {
    speak('Please provide your medical information for better healthcare assistance.');
  }, [speak]);
      ...
      <div className="w-full max-w-4xl relative z-10">
        <div className="text-center mb-10 animate-fade-in">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto opacity-90 leading-relaxed">
            {t('Please provide your Medical Information for Better Healthcare Assistance')}
          </p>
          <div className="flex items-center justify-center gap-2 mt-4">
            <Sparkles className="w-5 h-5 text-purple-500 animate-pulse" />
            <span className="text-sm text-purple-600 font-medium">Voice-enabled for easy input</span>
          </div>
        </div>
          ...
          {listening && activeVoiceField && (
            <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-20 bg-gradient-to-r from-red-500 to-pink-500 text-white px-6 py-3 rounded-full shadow-lg animate-pulse">
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold">Listening for: {activeVoiceField.replace(/_/g, ' ')}</span>
                <span className="text-sm opacity-90">{transcript || 'Speak now...'}</span>
              </div>
            </div>
          )}

          <form onSubmit={handleSubmit} className="p-8 sm:p-10 space-y-10 relative">
                ...
                <div className="relative group">
                  ...
                  {listening && activeVoiceField === 'name' && (
                    <div className="absolute -bottom-6 left-0 text-sm text-purple-600 flex items-center gap-2 bg-purple-50 px-3 py-1 rounded-full border border-purple-200">
                      ...
                      <span className="animate-pulse">Listening: {transcript || 'Speak now...'}</span>
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-8 animate-fade-in-up">
              ...
              <div className="space-y-6">
                {formData.emergencyContacts.map((contact, index) => (
                  <div key={index} className="bg-gradient-to-br from-indigo-50/50 to-blue-50/50 p-6 rounded-2xl border border-indigo-100/50 hover:border-indigo-200 transition-all duration-300 shadow-lg hover:shadow-xl backdrop-blur-sm group">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="relative group">
                        ...
                        <div className="absolute right-3 top-10">
                          <VoiceButton
                            listening={listening && activeVoiceField === `contactName_${index}`}
                            onClick={() => toggleVoiceInput(`contactName_${index}`)}
                            fieldName={`contact ${index + 1} name`}
                          />
                        </div>
                        {listening && activeVoiceField === `contactName_${index}` && (
                      ...
                      <div className="relative group">
                        ...
                        <div className="absolute right-3 top-10">
                          <VoiceButton
                            listening={listening && activeVoiceField === `contactNumber_${index}`}
                            onClick={() => toggleVoiceInput(`contactNumber_${index}`)}
                            fieldName={`contact ${index + 1} number`}
                          />
                        </div>
                        {listening && activeVoiceField === `contactNumber_${index}` && (
                          <div className="absolute -bottom-6 left-0 text-sm text-indigo-600 flex items-center gap-2 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-200">
                            <Mic className="w-4 h-4 animate-pulse" />
                            <span className="animate-pulse">Listening: {transcript || 'Speak now...'}</span>
                          </div>
            ...
            <div className="space-y-8 animate-fade-in-up">
              ...
              <div className="space-y-4 group">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center">
                  ...
                  <span className="text-gray-500 text-sm font-medium transform group-hover:scale-105 transition-transform">or</span>
                  <input
                    type="text"
                    name="currentMedicalStatus"
                    value={formData.currentMedicalStatus}

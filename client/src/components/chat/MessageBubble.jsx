...
export function MessageBubble({ message, isUser, isError, timestamp, index }) {
    ...
    return (
        ...
        >
            <div
                className={`flex items-start gap-3 max-w-[85%] sm:max-w-[75%] lg:max-w-[70%] ${isUser ? "flex-row-reverse" : "flex-row"
                    }`}
            >
                {/* Avatar */}
                <div
                    className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isUser
                        ? "bg-blue-500 text-white border border-neutral-400"
                        : isError
                ...
                {/* Message Bubble */}
                <div
                    className={`rounded-2xl px-4 py-3 shadow-sm transition-all duration-200 ${isUser
                ...
                >
                            ...
                            >
                                <ReactMarkdown
                                    components={{
                                        ...
                                        pre: ({ children }) => <pre className="bg-gray-200 dark:bg-gray-600 p-2 rounded text-xs font-mono overflow-x-auto mb-2">{children}</pre>,
                    ...
                    <div
                        className={`text-xs mt-2 opacity-70 ${isUser
                            ? "text-blue-100"
                            : isError
